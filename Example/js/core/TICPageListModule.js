...
var TICPageList = (function()
{
	...
	var observerCallback = function(message)
	{
		if(message.hasOwnProperty("statusUpdate") && message.statusUpdate.device == "app")
		{
			if(message.statusUpdate.type == "feedSelect")
			{
				...
				{
						$(".TICPageListContainer").css("width", "230px");
						$(".TICPageContentContainer").css("width", "760px");
						$(".TICPageListContainer").show();
				...
				else if (selectedFeedInternal.type == "TIC" &&!selectedFeedApplication.pageListDisplay)
				{
					$(".TICPageListContainer").hide();
					$(".TICPageContentContainer").css("width", "1000px");
				}
				else
				{
		...
		if( message.hasOwnProperty("tvPlayheadUpdate") && message.tvPlayheadUpdate.feedId == selectedFeedInternal.feedId)
		{
			if(selectedTvFeed == undefined || message.tvPlayheadUpdate.feedId != selectedTvFeed.feedId)
			{
				...
				lasTimestamp = message.tvPlayheadUpdate.progress;
			}
		...
		{
			if(pMessage.progress > nextPageTime && pMessage.progress >= lastTimestamp)
			{
					lastTimestamp = pMessage.progress;
	...
	var checkTvFeedLiveTime = function()
	{
		setTimeout(function(){
			...
			if(selectedTvFeed && showingSpoilers == "unlocked" && (selectedTvFeed.feedId != selectedFeedInternal.feedId
