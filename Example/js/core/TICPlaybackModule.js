...
var TICplaybackModule = (function(){
		...
		if(message.hasOwnProperty("statusUpdate"))
		{
			if(message.statusUpdate.type == "feedSelect" && message.statusUpdate.device == "app")
			{
				...
				if(selectedFeed.type == "TIC")
				{
					...
					nextAutoPage = 0;
					checkNextAutoPage(message.statusUpdate.progress);
				}
	...
	var checkNextAutoPage = function(progress)
	{
		...
		if(autoPages.length > 0 && autoPages.length-1 >= nextAutoPage && progress >= autoPages[nextAutoPage].time )
		{
			pageLoaded = autoPages[nextAutoPage].pageId;
